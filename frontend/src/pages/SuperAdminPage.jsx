...
import {
  ...
  HiOutlineCash,
  HiOutlineSearch,
  HiOutlineLockClosed // <--- ÍCONO NUEVO
} from "react-icons/hi";
...
function SuperAdminPage() {
  ...
  const handleRegisterPayment = async (companyId, companyName) => {
    ...
    if (
      !window.confirm(
        `¿Confirmas que ${companyName} pagó por ${months} mes(es)?\n\nEsto activará la empresa y extenderá su fecha de vencimiento.`,
      )
    )
      return;
    ...
    setCompanies((prev) =>
      prev.map((c) => {
        if (c.id === companyId) {
          ...
          return {
            ...c,
            modules: { ...currentModules, [moduleKey]: newValue },
          };
        }
        return c;
      }),
  ...
  const toggleStatus = async (companyId, currentStatus, companyName) => {
    ...
    try {
      await api.patch(`/super-admin/companies/${companyId}/status`, {
  ...
  const toggleFreeze = async (companyId, currentFrozenDate, companyName) => {
    const isFrozen = !!currentFrozenDate;
    const action = isFrozen ? "DESCONGELAR 🔥" : "CONGELAR ❄️";
    const message = isFrozen
      ? `¿Deseas ${action} la empresa "${companyName}"?\n\nLos datos dejarán de borrarse automáticamente.`
      : `¿Deseas ${action} la empresa "${companyName}"?\n\nSe creará un PUNTO DE RESTAURACIÓN AHORA MISMO.\nTodo lo que se cree a partir de este momento será borrado cada noche a las 23:55.`;

    if (!window.confirm(message)) return;

    try {
      await api.patch(`/super-admin/companies/${companyId}/freeze`, {
        frozen: !isFrozen,
      });
      fetchCompanies(); // Recargar para ver el cambio
      toast.success(
        `Empresa ${!isFrozen ? "CONGELADA" : "DESCONGELADA"} correctamente.`,
      );
    } catch (error) {
      toast.error("Error al cambiar estado de congelamiento.");
    }
  };

  const getPaymentStatus = (dueDate) => {
    if (!dueDate)
      return { color: "bg-gray-100 text-gray-500", text: "SIN FECHA" };
    ...
    if (diffDays < 0)
      return {
        color: "bg-red-100 text-red-700 font-bold animate-pulse",
    ...
    return {
      ...
      text: `Vence el ${due.toLocaleDateString()}`,
    };
  };

  if (loading)
    return <div className="p-10 text-center">Cargando Torre de Control...</div>;
      ...
      <div className="mb-8 bg-indigo-700 text-white p-8 rounded-2xl shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-indigo-100 mt-2 text-lg">
            Gestión Total de Empresas y Personal.
          </p>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        {filteredCompanies.map((company) => {
          ...
          return (
            <div
              key={company.id}
              className={`bg-white rounded-xl shadow-lg border-l-8 transition-all ${company.is_active ? "border-green-500" : "border-red-500"}`}
            >
              <div className="p-6">
                <div className="flex flex-col lg:flex-row justify-between items-start mb-6 border-b border-gray-100 pb-6">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span
                        className={`px-3 py-1 rounded text-xs border ${status.color}`}
                      >
                        {status.text}
                      </span>
                  ...
                  <div className="mt-4 lg:mt-0 lg:ml-6 flex flex-col gap-3 min-w-[200px]">
                    ...
                    <button
                      onClick={() =>
                        toggleFreeze(
                          company.id,
                          company.demo_frozen_at,
                          company.name,
                        )
                      }
                      className={`w-full py-2 px-4 rounded-lg font-bold text-sm shadow-sm flex items-center justify-center gap-2 transition-colors border ${
                        company.demo_frozen_at
                      ...
                      title={
                        company.demo_frozen_at
                          ? `Congelada desde: ${new Date(company.demo_frozen_at).toLocaleString()}`
                          : "Activar Modo Demo"
                      }
                    >
                      <HiOutlineLockClosed
                        className={`text-lg ${company.demo_frozen_at ? "animate-pulse" : ""}`}
                      />
                      {company.demo_frozen_at
                        ? "MODO DEMO (ON)"
                        : "MODO DEMO (OFF)"}
                    ...
                    <button
                      onClick={() =>
                        handleRegisterPayment(company.id, company.name)
                      }
                    ...
                    >
                      {company.is_active ? (
                        <>
                          <HiOutlineBan /> BLOQUEAR EMPRESA
                        </>
                      ) : (
                        <>
                          <HiOutlineCheckCircle /> ACTIVAR SERVICIO
                        </>
                      )}
                    </button>
                    ...
                    >
                      ...
                      {expandedCompanyId === company.id
                        ? "OCULTAR PERSONAL"
                        : `VER PERSONAL (${company.user_count})`}
                    </button>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-5 border-t border-gray-100">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <ModuleSwitch
                      ...
                      color="green"
                      onClick={() =>
                        toggleModule(company.id, "pos", company.modules?.pos)
                      }
                    />
                    <ModuleSwitch
                      label="Taller"
...
function ModuleSwitch({ label, icon, active, color, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all duration-200 ${colors[color]}`}
    >
      <div
        className={`text-xl mb-1 transition-transform group-hover:scale-110 ${isActive ? "text-white" : ""}`}
      >
        {icon}
      </div>
