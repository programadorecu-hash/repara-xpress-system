...
function WorkOrderPage() {
  ...
  const groupedOrders = isManager && workOrders.length > 0 ? groupOrdersByLocation(workOrders) : null;
  ...
  if (loading && workOrders.length === 0) return <p className="text-center text-gray-500">Cargando órdenes de trabajo...</p>;
    ...
    <>
      <div className="bg-white p-6 rounded-xl shadow-md border text-secondary">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={handleOpenCreateForm} className="bg-accent text-white font-bold py-2 px-4 rounded-lg">
            + Nueva Orden
          </button>
        </div>
        ...
        {isManager ? (
          <div className="space-y-8">
            {groupedOrders && Object.keys(groupedOrders).map(locationName => (
              <div key={locationName}>
                <h2 className="text-xl font-semibold mb-3 border-b pb-2">{locationName}</h2>
                <WorkOrderTable orders={groupedOrders[locationName]} formatDate={formatDate} onEdit={handleOpenEditForm} />
              </div>
            ))}
          </div>
        ) : (
          <WorkOrderTable orders={workOrders} formatDate={formatDate} onEdit={handleOpenEditForm} />
        )}
      </div>
...
const WorkOrderTable = ({ orders, formatDate, onEdit }) => (
  <div className="overflow-x-auto">
    {orders.length === 0 ? (
       <p className="text-center text-gray-400 py-4">No hay órdenes de trabajo para mostrar.</p>
    ) : (
            ...
            <tr key={order.id} className="border-b hover:bg-gray-50">
              ...
              <td className="py-3 px-4">
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                  order.status === 'RECIBIDO' ? 'bg-blue-200 text-blue-800' :
                  order.status === 'LISTO' ? 'bg-green-200 text-green-800' :
              ...
              <td className="py-3 px-4 text-center">
                {/* El botón ahora llama a la función onEdit con el ID de la orden */}
                <button onClick={() => onEdit(order.id)} className="text-blue-500 hover:underline">Ver/Editar</button>
              </td>
            </tr>
          ))}
        </tbody>
